            ...
            <div className="relative flex flex-col min-w-0 break-words bg-gray-900 border-2 border-violet-400 w-full mb-6 shadow-xl rounded-lg -mt-64">
                <div className="px-6">
                    <div className="flex flex-wrap justify-center">
                            <div className="w-full lg:w-3/12 px-4 lg:order-2 flex justify-center">
                                <div className="relative">
                                    <img src={`http://localhost:8000${user.profile_picture}`} alt="profile picture" className="shadow-xl rounded-full h-auto align-middle border-none relative -m-16 -ml-3 lg:-ml-3 mb-2"/>
                                </div>
                            </div>
                            <div className="w-full lg:w-4/12 px-4 lg:order-3 lg:text-right mt-2">
                                <div className="py-6 px-3 mt-32 sm:mt-0">
                                    <button onClick={()=>history('/residences/cabins')} className="bg-violet-400 active:bg-violet-500 hover:bg-violet-500 uppercase text-slate-100 font-bold horder:shadow-md shadow text-xs px-4 py-2 rounded outline-none focus:outline-none sm:mr-2 mb-1 ease-linear transition-all duration-150">
                                        New Booking
                                    </button>
                                </div>
                            </div>
                    ...
                    <div className="text-center mt-12">
                        <h3 className="text-4xl font-semibold leading-normal mb-2 text-slate-200 ">{user.first_name} {user.last_name}</h3>
                        <div className="text-sm leading-normal mt-0 mb-2 text-slate-300 font-bold uppercase">
                                <b>Username: </b>{user.username}
                        </div>
                        <div className="mb-2 text-slate-200 mt-2">
                                {user.email}
                        </div>
                    </div> 
                    <div className="mt-10 py-10 border-t border-slate-600 text-center">
                        <div className="flex flex-wrap justify-center">
                            ...
                            <button onClick={()=>setDisplay(!display)} className="font-normal text-violet-500">Show Bookings</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <BookingsDiv display={display}>
                    <div className="bg-gray-900 border-2 border-violet-400 w-full rounded-lg mb-2">
                        <h1 className="text-4xl font-semibold leading-normal mb-2 text-slate-200 text-center">My Bookings</h1>
                        {
                            (bookings.length ?
                                bookings.map((item)=>(
                                    <ProfileBookingsCard 
                            ...
                            :
                                <h1 className="text-center text-gray-800 font-bold text-7xl p-4">YOU DON'T HAVE ANY BOOKINGS</h1>
                            )
                        }
